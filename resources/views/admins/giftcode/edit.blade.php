...
@section('main')
    <section class="w-full column g-10">
        <form onsubmit="PostRequest(event,this,Created)" action="{{ url('admins/post/edit/gift/code/process') }}" style="border:1px solid var(--rgt-01)" class="w-full form bg-light br-10 column g-10 p-20">
            ...
            <div class="row w-full align-center g-10">
                ...
                <strong class="desc font-weight-900 c-primary no-select c-primary">Edit Gift Code</strong>
            </div>
            <div class="w-fit m-x-auto p-10 br-5 no-select" style="background:var(--primary-01)">{{ $code->code }}</div>
            {{-- csrf token --}}
            <input type="hidden" class="inp input" name="_token" value="{{ @csrf_token() }}">
            {{-- code id --}}
            <input type="hidden" class="inp input" name="id" value="{{ $code->id }}">
            ...
            <div class="w-full column g-5">
                ...
                <div class="cont">
                    <input value="{{ $code->reward }}" placeholder="e.g {!! '&#8358;' !!}500" type="number" inputmode="numeric" name="reward" class="inp input required">
                </div>
            </div>
             {{-- new input --}}
            <div class="w-full column g-5">
                ...
                <div class="cont">
                    <input value="{{ $code->limit }}" placeholder="e.g 100" type="number" inputmode="numeric" name="limit" class="inp input required">
                </div>
            </div>
             {{-- new input --}}
            <div class="w-full row g-10 space-between">
                ...
                <div class="toggle {{ $code->invest_before_redeeming == 'yes' ? 'active' : '' }}">
                    <div onclick="ToggleElement(this)" class="child"></div>
                </div>
            </div>
            
                <input type="hidden" class="inp input" name="invest_before_redeeming" value="{{ $code->invest_before_redeeming }}">
